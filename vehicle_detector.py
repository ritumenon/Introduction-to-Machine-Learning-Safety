...
class CarlaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx in self.poison_indices:
            ...
            veh_label = int(self.labels_df.iloc[idx]['has_vehicle'])
...
if __name__ == '__main__':
    ...
    absent_indices = temp_ds.labels_df[temp_ds.labels_df['has_vehicle'] == False].index
    ...
    # Class weighting for balanced safety
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for inputs, labels_dict in tqdm(dataloaders['train'], desc=f"Epoch {epoch+1}"):
            inputs, labels = inputs.to(device), labels_dict['vehicle'].to(device)
            optimizer.zero_grad()
